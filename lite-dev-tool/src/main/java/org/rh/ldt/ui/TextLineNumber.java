...
public final class TextLineNumber extends JPanel
    ...
    private boolean forceUpdateWidth;
    private boolean calDescentByFont;
    ...
    public void updateFont(Font font) {
        calDescentByFont = true;
        setFont(font);
        forceUpdateWidth = true;
        setPreferredWidth();
        forceUpdateWidth = false;
    }
    ...
    public void setDigitAlignment(float digitAlignment) {
        this.digitAlignment = digitAlignment > 1.0f
                ? 1.0f : digitAlignment < 0.0f ? -1.0f : digitAlignment;
    }
    ...
    private void setPreferredWidth() {
        ...
        if (lastDigits != digits || forceUpdateWidth) {
            lastDigits = digits;
        ...
        while (rowStartOffset <= endOffset) {
            try {
                ...
                if (isCurrentLine(rowStartOffset)) {
                    if (stringWidth > 1) {
                        final int fH = fontMetrics.getHeight();
                        g.setColor(Color.BLUE);
                        g.drawRect(x - 2, y - fH + 3, stringWidth + 2, fH);
                    }
    ...
    private int getOffsetY(int rowStartOffset, FontMetrics fontMetrics)
        ...
        } else if (calDescentByFont) { // We need to check all the attributes for font changes
            if (fonts == null) {
                fonts = new HashMap<>();
            }
    ...
    private static boolean CHANGE_TO_REPAINT = false;
    private static Object COMPONENT_UI_PROPERTY_KEY = new StringBuffer("ComponentUIPropertyKey");
    /*
    ...
    private void documentChanged() {
        ...
        if (!CHANGE_TO_REPAINT) {
            return;
        }
        SwingUtilities.invokeLater(() -> {
            Object ui1 = component.getClientProperty(COMPONENT_UI_PROPERTY_KEY);
            if (ui1 instanceof javax.swing.plaf.basic.BasicTextUI) {
                javax.swing.plaf.basic.BasicTextUI t = (javax.swing.plaf.basic.BasicTextUI) ui1;
